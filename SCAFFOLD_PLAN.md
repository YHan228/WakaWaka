...
| 1 | Schemas | `wakadecoder/schemas/*.py` | Medium |
...
| 8 | Runtime Core | `wakadecoder/classroom/*.py` | Medium |
| 9 | Runtime Viewer | `wakadecoder/viewer/*.py` | Medium |
...
## Phase 1: Schemas
**Goal**: Define all Pydantic models from `WakaDecoder_PROMPT_v2_PROMPTS.md`.

### Tasks
- [ ] Create `wakadecoder/schemas/annotation.py`
- [ ] Create `wakadecoder/schemas/curriculum.py`
- [ ] Create `wakadecoder/schemas/lesson.py`
- [ ] Create `wakadecoder/schemas/progress.py`
- [ ] Create `wakadecoder/schemas/__init__.py` (exports all)
- [ ] Write schema validation tests

### Verification
```bash
python -c "from wakadecoder.schemas import PoemAnnotation, LessonGraph, LessonContent, StudentProgress"
...
- [ ] Create `wakadecoder/utils/prompt_loader.py`

### Verification
```bash
python -c "from wakadecoder.utils.prompt_loader import load_prompt; p = load_prompt('annotate'); print(p['meta']['version'])"
```
...
python -c "
import pandas as pd
from wakadecoder.schemas import PoemAnnotation
...
python -c "
import json
from wakadecoder.schemas import LessonGraph
...
python -c "
import json, glob
from wakadecoder.schemas import LessonContent
...
**Note**: Progress table is stored separately in `~/.wakadecoder/progress.db` (user data, not content).
...
### Tasks
- [ ] Create `wakadecoder/classroom/loader.py`
- [ ] Create `wakadecoder/classroom/progress.py`
- [ ] Create `wakadecoder/classroom/navigator.py`

### Verification
```bash
python -c "
from wakadecoder.classroom import ClassroomLoader, Navigator
...
### Tasks
- [ ] Create `wakadecoder/viewer/lesson.py` (render lesson content)
...
```bash
python -c "
from wakadecoder.viewer import render_poem_with_ruby, render_lesson
# Test with mock data
"
```
...
- **Progress**: persists in `~/.wakadecoder/progress.db` (separate from content)
...
- [ ] Implement progress tracking (persist to ~/.wakadecoder/progress.db)
