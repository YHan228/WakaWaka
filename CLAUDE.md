...
## Key Documentation
- `WakaWaka_PROMPT_v2.md` — Full specification
- `WakaWaka_PROMPT_v2_PROMPTS.md` — LLM prompts & Pydantic schemas
- `WakaWaka_RESOURCES.md` — Corpus acquisition rules
- `SCAFFOLD_PLAN.md` — Phase-by-phase build plan

## Quick Start
```bash
# Activate conda (required for non-interactive shells / LLM agents)
source ~/miniconda3/etc/profile.d/conda.sh && conda activate wakawaka
...
- **Modular architecture** — UI in `app.py`, logic in `wakawaka/`
...
- Phase 1: `python -c "from wakawaka.schemas import *"` works
